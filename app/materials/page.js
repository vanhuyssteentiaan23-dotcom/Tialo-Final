...
function safeFilename(name) {
  const cleaned = name.normalize('NFKC').replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/-+/g, '-').replace(/^[-.]+|[-.]+$/g, '').slice(0, 160)
        ...
        {subjects.length === 0 ? (
          ...
          <>
            <div className="panel" style={{ marginTop: 32 }}>
              ...
              <div onDragOver={event => { event.preventDefault(); setDragging(true) }} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); addFiles(event.dataTransfer.files) }} onClick={() => fileInput.current?.click()} style={{ marginTop: 18, border: `1px dashed ${dragging ? 'rgba(69,230,161,.8)' : 'rgba(255,255,255,.18)'}`, background: dragging ? 'rgba(69,230,161,.08)' : 'rgba(255,255,255,.02)', borderRadius: 18, padding: '42px 24px', textAlign: 'center', cursor: 'pointer', transition: 'all .2s' }}>
                <div style={{ fontSize: 28, marginBottom: 10 }}>↑</div><strong>Drop files here or click to browse</strong><p className="muted" style={{ margin: '9px 0 0', fontSize: 12 }}>PDF, DOCX, PPTX, PNG, JPG or WEBP · Maximum 50 MB per file</p>
              ...
              {selectedFiles.length > 0 && <div style={{ marginTop: 18, display: 'grid', gap: 8 }}>
                {selectedFiles.map((file, index) => <div key={`${file.name}-${file.lastModified}`} className="card" style={{ padding: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><div style={{ minWidth: 0 }}><strong style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</strong><span className="muted" style={{ fontSize: 11 }}>{ACCEPTED_TYPES[file.type]} · {formatBytes(file.size)}</span></div><button type="button" className="btn secondary" onClick={event => { event.stopPropagation(); removeFile(index) }}>Remove</button></div>)}
