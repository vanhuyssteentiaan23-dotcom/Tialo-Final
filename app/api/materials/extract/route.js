...
export async function POST(request) {
  ...
  const { data: material, error: materialError } = await supabase.from('materials').select('id,user_id,file_name,mime_type,storage_path').eq('id', materialId).eq('user_id', user.id).maybeSingle()
  ...
  try {
    ...
    const { error: updateError } = await supabase.from('materials').update({ extracted_text: extractedText, extracted_at: new Date().toISOString(), processing_status: 'ready', processing_error: null }).eq('id', material.id).eq('user_id', user.id)
