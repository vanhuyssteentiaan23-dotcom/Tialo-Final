...
export async function POST(request) {
  ...
  if (authError || !user) {
    return NextResponse.json({
      error: 'Your session is invalid or expired. Please refresh the page and try again.',
    }, { status: 401 })
  }
  ...
  if (material.mime_type !== 'application/pdf') {
    return NextResponse.json({
      error: 'PDF extraction is enabled first. DOCX and PPTX extraction will be added next.',
    }, { status: 400 })
  }
  ...
  try {
    ...
    if (downloadError || !file) {
      throw new Error(downloadError?.message || 'Could not download the private material.')
    }
    ...
    if (!extractedText) {
      throw new Error('No selectable text was found in this PDF. A scanned/image-only PDF will need OCR.')
    }
