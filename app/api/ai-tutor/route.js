...
const KNOWN_STUDY_IMAGES=[
 {terms:['rna','ribonucleic'],images:[
  {title:'RNA structure — primary, secondary and tertiary structure',url:'https://commons.wikimedia.org/wiki/Special:FilePath/RNA_structure_(full).png?width=900',source:'Wikimedia Commons'},
  {title:'RNA strand structure',url:'https://commons.wikimedia.org/wiki/Special:FilePath/RNA_rna.svg?width=900',source:'Wikimedia Commons'},
  {title:'RNA chemical structure and 5′ to 3′ direction',url:'https://commons.wikimedia.org/wiki/Special:FilePath/Ribonucleic_acid_chemical_structure.svg?width=900',source:'Wikimedia Commons'},
  {title:'RNA transcription diagram',url:'https://commons.wikimedia.org/wiki/Special:FilePath/Transcription.svg?width=900',source:'Wikimedia Commons'},
  {title:'RNA translation / protein synthesis diagram',url:'https://commons.wikimedia.org/wiki/Special:FilePath/Protein_synthesis.svg?width=900',source:'Wikimedia Commons'}
 ]}
]
function knownStudyImages(query,count){const q=String(query||'').toLowerCase();const match=KNOWN_STUDY_IMAGES.find(x=>x.terms.some(t=>new RegExp('\\\\b'+t+'\\\\b','i').test(q)));return match?match.images.slice(0,count):[]}
...
export async function POST(request){
 ...
 const previousUser=history.slice().reverse().find(x=>x?.role==='user'&&typeof x.content==='string')?.content||'';const imageRequest=wantsImages(question);const imageCount=requestedImageCount(question);const imageTopic=(imageRequest?question+' '+previousUser:previousUser);let imageResults=imageRequest?knownStudyImages(imageTopic,imageCount):[];if(imageRequest&&imageResults.length<imageCount){const searched=await findStudyImages(imageTopic,'',imageCount);for(const img of searched){if(!imageResults.some(x=>x.url===img.url)&&/\\b(rna|ribonucleic|mrna|trna|transcription|translation|protein synthesis)\\b/i.test(img.title))imageResults.push(img);if(imageResults.length>=imageCount)break}}
