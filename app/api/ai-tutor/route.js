...
export async function POST(request) {
  ...
  const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/${process.env.GEMINI_TUTOR_MODEL || 'gemini-3.5-flash-lite'}:generateContent', {
    ...
    body: JSON.stringify({
      systemInstruction: { parts: [{ text: `You are TIALO AI Tutor. Answer the student entirely in ${language}. Keep scientific terms, formulas and proper nouns accurate. Teach clearly and step by step. Use ONLY the supplied study-material sources. Do not invent facts.\n\nSUPPLIED STUDY MATERIAL:\n${context}` }] },
...
export async function POST(request) {
  ...
  const answer = result?.candidates?.[0]?.content?.parts?.map(item => item.text || '').join('') || ''
