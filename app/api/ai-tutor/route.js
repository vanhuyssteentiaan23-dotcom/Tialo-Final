...
export async function POST(request){
 ...
 const answer=result?.candidates?.[0]?.content?.parts?.map(x=>x.text||'').join('')||'';if(!answer.trim())return NextResponse.json({error:'The AI Tutor returned an empty answer.'},{status:502});return NextResponse.json({answer})
}
