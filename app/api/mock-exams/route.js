...
function buildMaterialContext(materials, question = '') {
  ...
  return candidates.slice(0, 12).map((item, index) => `SOURCE ${index + 1} — ${item.title}\n${item.text}`).join('\n\n')
}
...
async function generateExam({ supabase, user, subjectId, count }) {
  ...
  for (const question of questions) {
    if (!question.prompt || !Array.isArray(question.options) || question.options.length !== 4 || !question.options.includes(question.correct_answer)) {
      return NextResponse.json({ error: 'The generated exam failed validation. Please try again.' }, { status: 502 })
    }
  }
  ...
  const { data: exam, error: examError } = await supabase.from('exam_attempts').insert({
    ...
    total_marks: count,
    status: 'in_progress',
  }).select('id,title,question_count,total_marks,status,created_at').single()
