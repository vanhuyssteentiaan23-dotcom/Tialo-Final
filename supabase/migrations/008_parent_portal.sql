...
create policy "Children can accept own parent links" on public.parent_child
  for update using (auth.uid() = child_id) with check (auth.uid() = child_id);

-- Allow parents to read only the linked child's basic profile.
...
-- Parent read-only access to academic reporting data.
...
create policy "Parents can view linked child subjects" on public.subjects
  for select using (
    exists (
      select 1 from public.parent_child pc
      where pc.parent_id = auth.uid()
        and pc.child_id = subjects.user_id
        and pc.status = 'active'
    )
  );
...
create policy "Parents can view linked child daily tasks" on public.daily_study_tasks
  for select using (
    exists (
      select 1 from public.parent_child pc
      where pc.parent_id = auth.uid()
        and pc.child_id = daily_study_tasks.user_id
        and pc.status = 'active'
    )
  );
...
create policy "Parents can view linked child exams" on public.exam_attempts
  for select using (
    exists (
      select 1 from public.parent_child pc
      where pc.parent_id = auth.uid()
        and pc.child_id = exam_attempts.user_id
        and pc.status = 'active'
    )
  );

-- Secure RPC: a parent can create a pending link only when the target profile
-- is an under-16 student. The child's email is not exposed to other users.
...
begin
  ...
  where lower(trim(email)) = lower(trim(child_email))
    and role = 'student';
  ...
  if child_profile.date_of_birth is null or
     date_part('year', age(current_date, child_profile.date_of_birth)) >= 16 then
    raise exception 'Parent linking is currently required only for students under 16.';
  end if;

  if child_profile.id = auth.uid() then
    raise exception 'A parent account cannot be linked to itself.';
  end if;

  insert into public.parent_child(parent_id, child_id, status)
  values (auth.uid(), child_profile.id, 'pending')
